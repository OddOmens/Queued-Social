...
-- The previous migration tried to use JWT claims which don't exist in cron context
-- This uses the proper authentication method for internal edge function calls

-- Ensure http extension is available
CREATE EXTENSION IF NOT EXISTS http WITH SCHEMA extensions;

-- Fix the process_scheduled_posts function with proper authentication
...
BEGIN
  ...
  -- Get the anon key from Vault (Supabase stores this in secrets)
  -- In Supabase, the edge function will use its own service role key
  -- We just need to provide any valid auth header for the request
  -- The edge function internally uses SUPABASE_SERVICE_ROLE_KEY from its environment
  ...
  -- The edge function doesn't actually validate auth when called internally
  -- It uses its own SUPABASE_SERVICE_ROLE_KEY from the environment
  BEGIN
...
COMMENT ON FUNCTION process_scheduled_posts() IS 'Processes scheduled posts by calling Edge Function via internal kong URL - Fixed auth issue';
